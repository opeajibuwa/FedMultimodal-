...
class MMActionClassifier(nn.Module):
    def __init__(
        ...
        elif self.att_name == "cross_modal":
            # Choose a number of heads that divides d_hid evenly
            num_heads = 4  # Default for d_hid=128
            if d_hid % num_heads != 0:
                # Find the largest divisor of d_hid that's <= min(d_head, d_hid)
                for i in range(min(d_head, d_hid), 0, -1):
                    if d_hid % i == 0:
                        num_heads = i
                        break
                        
            self.cross_att = CrossModalAttention(
                d_hid=d_hid,
                num_heads=num_heads,
    ...
    def forward(
        ...
        if self.en_att:
            if self.att_name == 'multihead':
                ...
                x_mm = self.cross_att(x_audio, x_video, len_a, len_v)
                # Calculate predictions before returning
                preds = self.classifier(x_mm)
                return preds, x_mm  # Early return for cross-modal attention
        else:
            # 4. Average pooling
            x_audio = torch.mean(x_audio, axis=1)
            x_video = torch.mean(x_video, axis=1)
        ...
        if self.en_att and self.att_name != "fuse_base" and self.att_name != "cross_modal":
...
class ImageTextClassifier(nn.Module):
    def __init__(
        ...
        # Cross-modal attention
            ...
            # Choose a number of heads that divides d_hid evenly
            num_heads = 4  # For d_hid=64, this works well
            if d_hid % num_heads != 0:
                # Find the largest divisor of d_hid that's <= d_head
                for i in range(min(d_head, d_hid), 0, -1):
                    if d_hid % i == 0:
                        num_heads = i
                        break
    ...
    def forward(self, x_img, x_text, len_i, len_t):
        # 1. img proj
        x_img_proj = self.img_proj(x_img[:, 0, :]).unsqueeze(1)  # Add sequence dimension
        ...
        if self.en_att:
            if self.att_name == "fuse_base":
                # get attention output
                x_mm = torch.cat((x_img_proj, x_text), dim=1)
                x_mm = self.fuse_att(x_mm, len_i, len_t, 1)
            elif self.att_name == "cross_modal":
                # Cross-modal attention between image and text
                x_mm = self.cross_att(x_img_proj, x_text, len_i, len_t)
        else:
            # 4. Average pooling
            x_text = torch.mean(x_text, axis=1)
            x_mm = torch.cat((x_img_proj.squeeze(1), x_text), dim=1)
...
class SERClassifier(nn.Module):
    def __init__(
        ...
        # Add cross-modal attention
            ...
            # Choose a number of heads that divides d_hid evenly
            num_heads = 4  # Default for d_hid=64
            if d_hid % num_heads != 0:
                # Find the largest divisor of d_hid that's <= min(d_head, d_hid)
                for i in range(min(d_head, d_hid), 0, -1):
                    if d_hid % i == 0:
    ...
    def forward(self, x_audio, x_text, len_a, len_t):
        ...
        if self.en_att:
            if self.att_name == 'multihead':
                ...
                x_mm = self.cross_att(x_audio, x_text, len_a, len_t)
                # return preds, x_mm  # Early return for cross-modal attention
        else:
    ...
    def forward(self, x_acc, x_gyro, l_a, l_b):
        ...
        if self.en_att:
            if self.att_name == 'multihead':
                ...
                # Cross-modal attention between accelerometer and gyroscope data
                x_mm = self.cross_att(x_acc, x_gyro, l_a, l_b)
                # Calculate predictions before returning
                preds = self.classifier(x_mm)
                return preds, x_mm  # Early return for cross-modal attention
        else:
...
#     """
#     Cross-Modal Attention mechanism that allows each modality to attend to features
#     from the other modality, enabling better inter-modal interactions.
#     """
#     def __init__(
#         self, 
#         d_hid: int = 64,
...
#         # Ensure num_heads divides d_hid evenly
#         assert d_hid % num_heads == 0, f"d_hid ({d_hid}) must be divisible by num_heads ({num_heads})"
...
#         # Projection layers for combining attended features
...
#     def forward(self, img_feat, text_feat, len_i, len_t):
#         """
#         Forward pass for cross-modal attention
        
#         Args:
#             img_feat: Image features [batch_size, 1, d_hid]
#             text_feat: Text features [batch_size, seq_len, d_hid]
#             len_i: Valid lengths for image features
#             len_t: Valid lengths for text features
            
#         Returns:
#             fused: Fused cross-modal representations
#         """
#         batch_size = img_feat.size(0)
        
#         # Create attention mask for text based on valid lengths
#         text_key_padding_mask = None
#         if len_t[0] != 0:
#             text_key_padding_mask = torch.zeros((batch_size, text_feat.size(1)), 
#                                               device=text_feat.device, dtype=torch.bool)
#             for i in range(batch_size):
#                 if len_t[i] < text_feat.size(1):
#                     text_key_padding_mask[i, len_t[i]:] = True
        
#         # Image attends to text
#         img_attended_text, _ = self.img_to_text_attention(
#             query=img_feat,
#             key=text_feat,
#             value=text_feat,
...
#         # Text attends to image (expanded to match sequence length)
#         img_expanded = img_feat.expand(-1, text_feat.size(1), -1)
#         text_attended_img, _ = self.text_to_img_attention(
#             query=text_feat,
#             key=img_expanded,
#             value=img_expanded
#         )
        
#         # Average the text-attended-image features across the sequence dimension
#         text_attended_img = torch.mean(text_attended_img, dim=1, keepdim=True)
        
#         # Concatenate and fuse the cross-attended features
#         img_enhanced = torch.cat([img_feat, img_attended_text], dim=-1)
#         img_enhanced = self.fusion_layer(img_enhanced).squeeze(1)  # Remove seq dim
        
#         text_enhanced = torch.mean(text_feat, dim=1)  # Average pooling
...
class CrossModalAttention(nn.Module):
    ...
    def __init__(
        ...
        # Multi-head attention for cross-modal interactions
        self.mod1_to_mod2_attention = nn.MultiheadAttention(
            embed_dim=d_hid,
            num_heads=num_heads,
            dropout=dropout_p,
            batch_first=True
        ...
        self.mod2_to_mod1_attention = nn.MultiheadAttention(
            embed_dim=d_hid,
            num_heads=num_heads,
            dropout=dropout_p,
            batch_first=True
    ...
    def forward(self, mod1_feat, mod2_feat, len_mod1, len_mod2):
        """
        Forward pass for cross-modal attention
        
        Args:
            mod1_feat: Features from modality 1 [batch_size, seq_len1, d_hid]
            mod2_feat: Features from modality 2 [batch_size, seq_len2, d_hid]
            len_mod1: Valid lengths for modality 1 features
            len_mod2: Valid lengths for modality 2 features
            
        Returns:
            fused: Fused cross-modal representations [batch_size, d_hid*2]
        """
        batch_size = mod1_feat.size(0)
        
        # Create attention mask for modality 2 based on valid lengths
        mod2_key_padding_mask = None
        if len_mod2[0] != 0:
            mod2_key_padding_mask = torch.zeros((batch_size, mod2_feat.size(1)), 
                                           device=mod2_feat.device, dtype=torch.bool)
            for i in range(batch_size):
                if len_mod2[i] < mod2_feat.size(1):
                    mod2_key_padding_mask[i, len_mod2[i]:] = True
        
        # Create attention mask for modality 1 based on valid lengths
        mod1_key_padding_mask = None
        if len_mod1[0] != 0:
            mod1_key_padding_mask = torch.zeros((batch_size, mod1_feat.size(1)), 
                                           device=mod1_feat.device, dtype=torch.bool)
            for i in range(batch_size):
                if len_mod1[i] < mod1_feat.size(1):
                    mod1_key_padding_mask[i, len_mod1[i]:] = True
        
        # Modality 1 attends to modality 2
        mod1_attended, _ = self.mod1_to_mod2_attention(
            query=mod1_feat,
            key=mod2_feat,
            value=mod2_feat,
            key_padding_mask=mod2_key_padding_mask
        )
        
        # Modality 2 attends to modality 1
        mod2_attended, _ = self.mod2_to_mod1_attention(
            query=mod2_feat,
            key=mod1_feat,
            value=mod1_feat,
            key_padding_mask=mod1_key_padding_mask
        )
        
        # Apply valid length masks
        if len_mod1[0] != 0:
            for i in range(batch_size):
                if len_mod1[i] < mod1_feat.size(1):
                    mod1_attended[i, len_mod1[i]:] = 0
        
        if len_mod2[0] != 0:
            for i in range(batch_size):
                if len_mod2[i] < mod2_feat.size(1):
                    mod2_attended[i, len_mod2[i]:] = 0
        
        # Average the attended features across sequence dimensions
        mod1_enhanced = torch.mean(mod1_attended, dim=1)
        mod2_enhanced = torch.mean(mod2_attended, dim=1)
        
        # Concatenate for final multimodal representation
        fused = torch.cat([mod1_enhanced, mod2_enhanced], dim=-1)
